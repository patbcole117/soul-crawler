...
export function newPlayer(name = 'Wanderer') {
  const p = {
    name,
    level: 1,
    xp: 0,
...
export function addXp(player, amount) {
  ...
  if (levelsGained > 0) {
    player.hp = maxStats(player).maxHp;
  }
  return levelsGained;
}
...
export function clampHp(player) {
  const max = maxStats(player).maxHp;
  player.hp = Math.max(0, Math.min(player.hp, max));
}
